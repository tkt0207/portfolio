...
function call_fav_setting(e) {
    ...
    // 対象テーブル取得
    const table_names = DB.get_tables(fav_setting.d_name);
    // テーブル選択リスト更新
...
function get_table_data() {
    ...
    // console.log(`TABLE名：${table_name}`);
    // console.log(`種別：${table_type}`);
    // console.log(`件数：${view_count}`);
    // console.log(`ページ：${page_no}`);
    // console.log(`条件：${filters}`);
    // console.log(`並び替え：${sorts}`);

    //TODO: DBアクセスし、情報取得する

    // テーブルデータ更新
    table_data = DB.get_table_data(db_name, table_name, table_type, view_count, page_no, filters, sorts);
...
//   DB切り替えイベントを設定する
//=============================================
function change_db(e){
    // ターゲット取得
    const target = e.currentTarget;
    const db_name = target.value;

    // 対象テーブル取得
    const table_names = DB.get_tables(db_name);
...
function set_switch_page_event(){
    ...
    // ページ選択リスト
    $select_page.addEventListener("change", () =>{
        get_table_data();
        table_update();
    })
    // 表示件数
