...
// 隠しスタイル名
const STYLE_NAME_LIST_HIDDEN = 'simple_list_picker_hidden';
...
function Simple_list(props) {
    ...
    //------------------------------------------------
    return(
        <div className='simple_list_block'>
            <select name={list_name} id={list_id} ref={select_list} className='simple_list'
                onChange={change_event}>
                {dummy_list}
            </select>

            <button className='simple_list_button' ref={list_button}
                onClick={list_display_switch}></button>
            <div className={`simple_list_picker ${list_class.join(' ')}`}>
                {list}
            </div>
        </div>
